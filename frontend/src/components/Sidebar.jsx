      ...
      <div className='flex-1 overflow-y-auto p-2 space-y-1'>
        {filteredChats.map((chat) => (
          <div
            ...
            className={`group flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer text-sm transition-colors ${currentChat?.id === chat.id ? 'bg-indigo-900/30 border border-indigo-800' : 'hover:bg-gray-800 border border-transparent'}`}
          >
            <div className='flex items-center gap-2 min-w-0'>
              <VscComment className='w-3.5 h-3.5 text-gray-500 flex-shrink-0' />
